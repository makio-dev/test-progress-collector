...
| AG列 (33列目) | 横展開有無 | 有/無 |
| AH列 (34列目) | 横展開先 | テキスト |
| AI列 (35列目) | 横展開完了予定日 | 日付形式 |
| AJ列 (36列目) | 横展開完了日 | 日付形式 |
...
| セクション | 内容 |
|-----------|------|
| 1. 欠陥サマリー | チーム別の未完了・遅延・滞留・予定超過・横展開未完了の件数 |
...
| 項目 | カテゴリ | 条件 |
|------|---------|------|
| 未完了 | 全般 | 検証日(AM列)が空のレコード |
| 遅延 | 全般 | 調査予定日/対応予定日を超過し未完了のレコード |
| 滞留(7日超) | 全般 | 発見日から7日以上経過 かつ 未完了 |
| 調査予定超過 | 予定超過 | 調査予定日 < 基準日 かつ 調査完了日が空 |
| 対応予定超過 | 予定超過 | 対応予定日 < 基準日 かつ 対応日が空 |
| 横展開/検証超過 | 予定超過 | 横展開完了予定日 < 基準日 かつ 横展開完了日が空 |
| 横展開未完了 | 横展開 | 横展開有無が"有" かつ 横展開完了日が空 |

#### セクション2: 検出状況の参照

- **新規検出**: ダッシュボードシートの基準日を参照し、前営業日〜基準日に発見された欠陥数
- **週検出**: ダッシュボードシートの週範囲（From/To）を参照し、期間内に発見された欠陥数
- **累積検出**: チーム全レコード数
