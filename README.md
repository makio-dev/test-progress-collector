...
- **P系（テスト消化）**: 未実施テストケースの残数バーンダウン（計画・実績）。ダッシュボードの **「明細」シート** の `実施者_予定` / `実施者_実績` をベースに集計。母数（テストケース総数）は **ダッシュボードの「総数」と同じ定義＝実施予定が入っている明細のみ**（実施予定が空の行は対象外）。
...
生成されるExcelは **数式駆動** です。`入力データ`・`欠陥データ` シートは元データをコピーした編集可能リストで、グラフはこれらを **列全体参照** しています。そのため、**行の削除・追加どちらも自動でグラフに反映** されます。

`欠陥データ` シートには **ダッシュボードの「欠陥詳細_ALL」が全項目（チーム名・対応状況・件名・発見日・緊急度・原因 …）そのままコピー** されるため、どの欠陥を外すかを人が判断できます。特定の欠陥を集計から外したい場合は **該当行を削除**、欠陥を足したい場合は **末尾に行を追加** して再オープンするだけで、累積欠陥カーブとグラフが自動的に再計算されます（係数や基準日も `パラメータ` シートで直接編集可能）。

> 補足: グラフの **横軸（日付の期間）だけは生成時に固定** されます。期間そのものを延ばす場合は `--start-date` / `--end-date` を指定して再生成してください。

### グラフの見方

- **基準日**に薄いグレーの縦線マーカーが入ります（実績はこの線まで、右側は予測区間）。
- グラフの下に **「凡例（各線が示すもの）」テーブル**（色見本・線種・軸・意味）を出力します。各線が何を表すかはこの表で確認できます。
...
| `--total-case` | 実施予定ありの件数 | テストケース総数（既定はダッシュボード「総数」と同義） |

### 生成シート構成

| シート名 | 区分 | 内容 |
|----------|------|------|
| パラメータ | 入力（編集可） | 開始日/終了日/基準日/テストケース総数（＝実施予定ありの件数）/B系係数/予測倍率。`B_Plan_Final` 等は係数からの数式 |
| 入力データ | 入力（編集可） | テストケースの実施予定日・実績日（P系の元データ） |
| 欠陥データ | 入力（編集可） | ダッシュボード「欠陥詳細_ALL」の全項目コピー（B系は発見日を参照）。行削除で集計除外 |
