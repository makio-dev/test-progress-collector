...
`欠陥データ` シートには **ダッシュボードの「欠陥詳細_ALL」が全項目（チーム名・対応状況・件名・発見日・緊急度・原因 …）そのままコピー** されるため、どの欠陥を外すかを人が判断できます。特定の欠陥を集計から外したい場合は、**該当行を削除して再オープンするだけ** で、累積欠陥カーブとグラフが自動的に再計算されます（係数や基準日も `パラメータ` シートで直接編集可能）。
